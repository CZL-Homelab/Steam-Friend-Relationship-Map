...
- 抓取器会按请求批次一次读取 Kùzu 或 Neo4j 好友列表缓存，并自动忽略不完整的旧缓存后重新抓取。The crawler reads each Kùzu or Neo4j friend-list cache batch with one query and refetches incomplete legacy cache entries.
