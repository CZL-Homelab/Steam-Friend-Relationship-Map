...
| Steam Web API Key | 用来调用公开 Steam Web API |
...
### 第 5 步：填写 Steam API Key

打开 `.env`，把第 4 步复制到的 Key 填进去：
...
如果你在 Neo4j Desktop 里改过 Bolt 端口或用户名，就按你的实际配置填写。
...
| `NEO4J_PASSWORD` | Neo4j Desktop 中设置的数据库密码 |
...
4. Steam 状态显示正常。
5. Neo4j 状态显示正常。
6. 如果 Neo4j 失败，先确认 Neo4j Desktop 数据库是否已经 Start。
7. 如果 Steam 失败，先确认 `.env` 里的 `STEAM_API_KEY` 是否填写正确。
