...
def main():
    parser = argparse.ArgumentParser(
        description="Steam Friend Relationship Map Database Migration Tool"
    )
    parser.add_argument(
        "--from-engine",
        choices=["kuzu", "neo4j"],
    ...
    parser.add_argument(
        "--project-id",
        help="Specify a single project ID to migrate. If omitted, all projects will be migrated.",
    )
    ...
    parser.add_argument(
        "--neo4j-password",
        help="Override Neo4j password (will prompt if omitted and needed)",
    )
    ...
    if args.from_engine == "kuzu":
        ...
        src_repo = KuzuRepositoryImpl(
            db_path=db_path, buffer_pool_size_gb=settings.kuzu_buffer_pool_size_gb
        )
    else:
